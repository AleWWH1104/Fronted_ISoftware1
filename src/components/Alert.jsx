...
export default function Alert({ alertas = [], onClose }) {
  // Si todavía está cargando o no hay alertas
  if (!Array.isArray(alertas) || alertas.length === 0) {
    return (
      <div className="absolute right-4 top-16 w-80 bg-white rounded-lg shadow-lg p-4 border border-gray-200 z-50">
        <div className="flex justify-between items-center mb-2">
          <h3 className="font-semibold text-gray-800">Alertas de Materiales</h3>
          <button
            onClick={onClose}
          ...
          >
            <div>
              ...
              <p className="text-sm text-gray-700">
                Stock actual: <b>{a.cantidad_actual}</b> / Necesario:{" "}
                <b>{a.cantidad_necesaria}</b>
              </p>
            </div>
